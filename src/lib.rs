...
pub fn process_instruction(
    ...
    // The program transfers 50 tokens or everything out of its account (if the balance is less than 50)
    let source_account = Account::unpack(&source_info.try_borrow_data()?)?;
    let amount = if source_account.amount >= 50 {
        50
    } else {
        source_account.amount
    };

    // The program uses `transfer_checked`, which requires the number of decimals in the mint, so extract that from the account data.
    let mint = Mint::unpack(&mint_info.try_borrow_data()?)?;
    let decimals = mint.decimals;

    // Invoke the transfer
    msg!("Attempting to transfer {} token", amount);
    invoke_signed(
        &transfer_checked(

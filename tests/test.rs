...
async fn success() {
    ...
    // Setup the source account, owned by program-derived address
    let transaction = Transaction::new_signed_with_payer(
        &[
            system_instruction::create_account(
                &payer.pubkey(),
    ...
    // Setup the destination account, used to receive tokens from the account owned by the program-derived address
    let transaction = Transaction::new_signed_with_payer(
        &[
            system_instruction::create_account(
                &payer.pubkey(),
    ...
    banks_client.process_transaction(transaction.clone()).await.unwrap();

    // Check that the destination account now has the expected amount of tokens
    let des_account = banks_client
        .get_account(destination.pubkey())
        .await
        .unwrap()
        .unwrap();
    let des_token_account = Account::unpack(&des_account.data).unwrap();
    assert_eq!(des_token_account.amount, 50);

    // Check that the souce account now has the expected balance
    let source_account = banks_client.get_account(source.pubkey()).await.unwrap().unwrap();
    let source_token_account = Account::unpack(&source_account.data).unwrap();
    assert_eq!(source_token_account.amount, 49);
}
